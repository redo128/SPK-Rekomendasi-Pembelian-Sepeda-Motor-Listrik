...
class PerhitunganPembeli extends Controller
{
    ...
    {
        $temp_data_check_sepeda_listrik=SepedaListrik::where('tipe','sepeda listrik')->get();
        $temp_data_check_sepeda_listrik_count=SepedaListrik::where('tipe','sepeda listrik')->count();
        $temp_data_check_sepeda_motor_listrik=SepedaListrik::where('tipe','sepeda motor listrik')->get();
        $temp_data_check_sepeda_motor_listrik_count=SepedaListrik::where('tipe','sepeda motor listrik')->count();
        $temp_data_check=SepedaListrik::all();
        if($temp_data_check_sepeda_listrik->isEmpty()){
            return redirect()->route('sepeda_sa.index')->with('success', 'Data Sepeda Listrik Belum Ada');   
        }elseif($temp_data_check_sepeda_motor_listrik->isEmpty()){
            return redirect()->route('sepeda_sa.index')->with('success', 'Data Sepeda Motor Listrik Belum Ada'); 
        }elseif($temp_data_check_sepeda_listrik_count==1){
            return redirect()->route('sepeda_sa.index')->with('success', 'Data Sepeda Listrik Hanya 1'); 
        }elseif($temp_data_check_sepeda_motor_listrik_count==1){
            return redirect()->route('sepeda_sa.index')->with('success', 'Data Sepeda Motor Listrik Hanya 1'); 
        }elseif($temp_data_check->isEmpty()){
